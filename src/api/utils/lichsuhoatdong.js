import LichSuHoatDong, {
  ChiTietLichSuHistory,
} from "../resources/lichsuhoatdong/lichsuhoatdong.model";
...
export const saveLichSuHoatDong = async (user_id, type, data, urltrang) => {
  ...
  try {
    ...
    if (type === 3) {
      ...
      let lichsuhoatdong_update = await LichSuHoatDong.find({
        doituong_id: data._id,
      });
      lichsuhoatdong_update.map(async (e) => {
        await LichSuHoatDong.findByIdAndUpdate(e._id, {
          urltrang: `/${e.urltrang.split("/")[1]}`,
        });
      });
    }
    let datalichsu = {
...
function getTieuDe(data, urltrang) {
  ...
  if (urltrang === "danhmuc-tintuc") {
    ...
    tieude =
      "gói dịch vụ " +
      data.goidichvu_id.tengoi +
      " của bệnh nhân " +
      data.benhnhan_id.hoten;
    url = "/" + urltrang + "/" + data._id;
