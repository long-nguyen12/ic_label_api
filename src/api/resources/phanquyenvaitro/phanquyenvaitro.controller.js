...
export default {
  ...
  async update(req, res) {
    try {
      const { id } = req.params;
      const data = await PhanQuyenVaiTro.findByIdAndUpdate(id, req.body, { new: true })
      if (!data) {
  ...
  async postTrang(req, res){
    try{
      const {id} = req.params
      var phanquyenvaitro = await PhanQuyenVaiTro.findOne({is_deleted: false, _id: id});
  ...
  async putTrang(req, res){
    try{
      ...
      var phanquyenvaitro = await PhanQuyenVaiTro.findOne({is_deleted: false, _id: id_PhanQuyenVaiTro});
